...
class DatasetEvaluationReport:
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        if self.deidentification_profile != "hipaa-expert-aggregate":
...
def evaluate_dataset_manifest(
    ...
    if normalized_profile not in {"standard", "hipaa-expert-aggregate"}:
        raise ValueError("deidentification_profile must be one of: hipaa-expert-aggregate, standard")
    if normalized_profile == "hipaa-expert-aggregate":
        if output_modes != ("aggregate",):
            raise ValueError("hipaa-expert-aggregate dataset evaluation requires exactly the aggregate output mode")
        if visualize:
            raise ValueError("hipaa-expert-aggregate dataset evaluation does not support visualization")
    ...
    for offset, sample in enumerate(samples):
        ...
        sample_dir = root / (f"sample_{offset:04d}" if normalized_profile == "hipaa-expert-aggregate" else sample_id)
        ...
        if normalized_profile == "hipaa-expert-aggregate":
            aggregate_report = _read_json(sample_dir / "aggregate_report.json")
            retention_report = _read_json(sample_dir / "retention_report.json")
            processed_frames = int(aggregate_report.get("utility", {}).get("processed_frame_count") or 0)
            retention_passed = retention_report.get("passed")
            coverage = aggregate_report.get("utility", {}).get("pose_frame_coverage")
        else:
